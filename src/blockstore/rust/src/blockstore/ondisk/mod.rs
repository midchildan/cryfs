...
impl BlockStore2 for OnDiskBlockStore {
    ...
    fn load(&self, id: &BlockId) -> Result<Option<Vec<u8>>> {
        ...
        match std::fs::read(&path) {
            ...
            Err(err) => Err(err).with_context(|| format!("Failed to open block file at {}", path.display())),
            Ok(file_content) => {
                let block_content = _check_and_remove_header(&file_content)
                    .with_context(|| format!("Failed to parse file contents of block at {}", path.display()))?
                    .to_vec();
    ...
    fn num_blocks(&self) -> Result<u64> {
        let mut count = 0;
        self._for_each_block_file(&mut |_| -> Result<()> {count += 1; Ok(())});
        Ok(count)
    }
    ...
    fn all_blocks(&self) -> Result<Box<dyn Iterator<Item=BlockId>>> {
        ...
        self._for_each_block_file(&mut |file| {
            ...
            let blockid_prefix = path.parent()
                .unwrap_or_else(|| panic!("Block file path `{}` should have a parent component", path.display()))
                .file_name()
                .unwrap_or_else(|| panic!("Block file path `{}` should have a valid parent component", path.display()))
                .to_str()
                .unwrap_or_else(|| panic!("Block file path `{}` should be valid UTF-8", path.display()));
            let blockid_nonprefix = path.file_name()
                .unwrap_or_else(|| panic!("Block file path `{}` should have a valid last component", path.display()))
                .to_str()
                .unwrap_or_else(|| panic!("Block file path `{}` should be valid UTF-8", path.display()));
            assert_eq!(PREFIX_LEN, blockid_prefix.len(), "Block file path should have a prefix len of {} but was {:?}", PREFIX_LEN, blockid_prefix);
            assert_eq!(NONPREFIX_LEN, blockid_nonprefix.len(), "Block file path should have a nonprefix len of {} but was {:?}", NONPREFIX_LEN, blockid_nonprefix);
...
impl OnDiskBlockStore {
    fn _block_path(&self, blockId: &BlockId) -> PathBuf {
        let blockIdStr = blockId.to_hex();
        assert!(blockIdStr.chars().all(|c| _is_allowed_blockid_character(c)), "Created invalid blockIdStr");
        return self.basedir.join(&blockIdStr[..PREFIX_LEN]).join(&blockIdStr[PREFIX_LEN..]);
    }

    fn _for_each_block_file(&self, callback: &mut impl FnMut(&DirEntry) -> Result<()>) -> Result<()> {
        let mut count = 0;
        for subdir in self.basedir.read_dir()? {
            let subdir = subdir?;
            if subdir.metadata()?.is_dir() && subdir.file_name().len() == PREFIX_LEN && subdir.file_name().to_str().ok_or_else(|| anyhow!("Invalid UTF-8 in path in base directory: {}", subdir.path().display()))?.chars().all(|c| _is_allowed_blockid_character(c)) {
                for blockfile in subdir.path().read_dir()? {
                    let blockfile = blockfile?;
                    if blockfile.file_name().len() == NONPREFIX_LEN && blockfile.file_name().to_str().ok_or_else(|| anyhow!("Invalid UTF-8 in path in base directory: {}", blockfile.path().display()))?.chars().all(|c| _is_allowed_blockid_character(c)) {
                        callback(&blockfile)?;
                    }
                }
            }
...
fn _store(path: &Path, data: &[u8]) -> Result<()> {
    _create_dir_if_doesnt_exist(path.parent().expect("Block file path should have a parent directory"))
        .with_context(|| format!("Failed to create parent directory for block file at {}", path.display()))?;
...
mod tests {
    ...
    #[test]
    fn test_block_path() {
        let blockStore = OnDiskBlockStore::new(Path::new("/base/path").to_path_buf());
        assert_eq!(Path::new("/base/path/2AC/9C78D80937AD50852C50BD3F1F982"), blockStore._block_path(&BlockId::from_data(&hex::decode("2AC9C78D80937AD50852C50BD3F1F982").unwrap())));
    }
    ...
    #[test]
    fn test_prefix() {
        assert!(_data_starts_with(FORMAT_VERSION_HEADER, FORMAT_VERSION_HEADER_PREFIX));
    }
}